...
namespace loguru
{
	...
	// ------------------------------------------------------------------------------
	/**
 * @brief Determines if the terminal supports colored output.
 *
 * This function checks whether the terminal environment supports colored text,
 * which can be used to enhance the readability of log messages.
 *
...
#define VTSEQ(ID) ("\x1b[" #ID "m")
	...
	/**
 * @brief Returns the ANSI escape sequence for black text.
 *
 * This function checks if the terminal supports color output. If color is enabled, it returns the escape sequence
 * that sets the terminal text color to black; otherwise, it returns an empty string.
 *
 * @return const char* ANSI escape sequence for setting text to black, or an empty string if colors are not supported.
 */
const char* terminal_black()      { return s_terminal_has_color ? VTSEQ(30) : ""; }
	/**
 * @brief Retrieves the ANSI escape sequence for red text.
 *
 * This function returns the terminal escape sequence to set text color to red if color support is enabled.
 * If the terminal does not support colored output, it returns an empty string.
 *
 * @return const char* The red text escape sequence or an empty string if colors are disabled.
 */
const char* terminal_red()        { return s_terminal_has_color ? VTSEQ(31) : ""; }
	/**
 * @brief Returns the ANSI escape sequence for green text.
 *
 * If terminal color support is enabled, this function returns the escape sequence
 * to switch terminal output to green. Otherwise, it returns an empty string.
 *
 * @return const char* The ANSI escape sequence for green text, or an empty string if terminal color is not supported.
 */
const char* terminal_green()      { return s_terminal_has_color ? VTSEQ(32) : ""; }
	/**
 * @brief Retrieves the ANSI escape sequence for yellow text.
 *
 * Returns the ANSI escape sequence (VT sequence) for yellow if the terminal supports color output.
 * If color output is not enabled, an empty string is returned.
 *
 * @return const char* The ANSI escape code for yellow text or an empty string if colors are disabled.
 */
const char* terminal_yellow()     { return s_terminal_has_color ? VTSEQ(33) : ""; }
	/**
 * @brief Returns the terminal escape sequence for blue text.
 *
 * If terminal color support is enabled, this function returns the ANSI escape sequence
 * for setting the text color to blue. Otherwise, it returns an empty string.
 *
 * @return const char* ANSI escape sequence for blue or an empty string if colors are not supported.
 */
const char* terminal_blue()       { return s_terminal_has_color ? VTSEQ(34) : ""; }
	/**
 * @brief Retrieves the ANSI escape sequence for purple text.
 *
 * If the terminal supports colored output, this function returns the ANSI escape code for purple (color code 35).
 * Otherwise, it returns an empty string.
 *
 * @return const char* The ANSI escape sequence for purple, or an empty string if colors are not supported.
 */
const char* terminal_purple()     { return s_terminal_has_color ? VTSEQ(35) : ""; }
	/**
 * @brief Returns the ANSI escape sequence for cyan color.
 *
 * If the terminal supports color, this function returns the ANSI escape sequence (generated by VTSEQ(36))
 * to set the text color to cyan. Otherwise, it returns an empty string.
 *
 * @return const char* Pointer to the cyan escape sequence if supported; otherwise, an empty string.
 */
const char* terminal_cyan()       { return s_terminal_has_color ? VTSEQ(36) : ""; }
	/**
 * @brief Retrieves the escape sequence for light gray text.
 *
 * If the terminal supports color output, this function returns the ANSI escape sequence that
 * sets the text color to light gray. Otherwise, it returns an empty string.
 *
 * @return const char* ANSI escape sequence for light gray text if supported, or an empty string.
 */
const char* terminal_light_gray() { return s_terminal_has_color ? VTSEQ(37) : ""; }
	/**
 * @brief Retrieves the ANSI escape sequence to set terminal text to white.
 *
 * If the terminal supports color, this function returns the ANSI escape sequence for white text.
 * Otherwise, it returns an empty string.
 *
 * @return const char* ANSI escape sequence for white text, or an empty string if color is not supported.
 */
const char* terminal_white()      { return s_terminal_has_color ? VTSEQ(37) : ""; }
	/**
 * @brief Provides the ANSI escape sequence for light red text.
 *
 * Returns the ANSI escape sequence that enables light red coloring in terminal output if color support is enabled,
 * otherwise returns an empty string.
 *
 * @return const char* The ANSI escape sequence for light red text, or an empty string if colors are not supported.
 */
const char* terminal_light_red()  { return s_terminal_has_color ? VTSEQ(91) : ""; }
	/**
 * @brief Returns the terminal escape sequence to dim text output.
 *
 * This function returns the dimming VT sequence (via VTSEQ(2)) if terminal colors are enabled.
 * Otherwise, it returns an empty string, indicating that no dimming effect should be applied.
 *
 * @return const char* A VT escape sequence for dimming text, or an empty string if not supported.
 */
const char* terminal_dim()        { return s_terminal_has_color ? VTSEQ(2)  : ""; }

	/**
 * @brief Retrieves the terminal bold formatting sequence.
 *
 * Returns an ANSI escape sequence for bold text if terminal color support is enabled.
 * Otherwise, returns an empty string.
 *
 * @return const char* Bold formatting code or an empty string when colors are disabled.
 */
	const char* terminal_bold()       { return s_terminal_has_color ? VTSEQ(1) : ""; }
	/**
 * @brief Retrieves the terminal escape sequence for underlined text.
 *
 * If the terminal supports color, this function returns the VT escape sequence
 * to enable underlined text formatting. Otherwise, it returns an empty string.
 *
 * @return A pointer to a C-style string containing the escape sequence for underlining,
 *         or an empty string if underlined text is not supported.
 */
const char* terminal_underline()  { return s_terminal_has_color ? VTSEQ(4) : ""; }

	/**
 * @brief Retrieves the terminal reset escape sequence.
 *
 * If color support is enabled in the terminal, this function returns the ANSI escape sequence (via VTSEQ(0))
 * to reset terminal formatting after colored output. Otherwise, it returns an empty string.
 *
 * @return const char* A pointer to the reset sequence or an empty string if color support is disabled.
 */
	...
	/**
 * @brief Retrieves the FILE pointer from a FileAbs instance.
 *
 * This inline function casts the provided generic pointer to a FileAbs pointer and returns
 * its associated file pointer stored in the 'fp' member. The caller must ensure that the
 * user_data argument points to a valid FileAbs instance.
 *
 * @param user_data A pointer to a FileAbs structure.
 * @return FILE* The file stream pointer managed by the FileAbs instance.
 */
inline FILE* to_file(void* user_data) { return reinterpret_cast<FileAbs*>(user_data)->fp; }
#else
	/**
 * @brief Converts a generic pointer to a FILE pointer.
 *
 * This inline helper function reinterprets the provided pointer as a FILE pointer,
 * enabling its use in file-based logging operations.
 *
 * @param user_data Pointer expected to reference a FILE object.
 * @return FILE* The resulting FILE pointer.
 */
inline FILE* to_file(void* user_data) { return reinterpret_cast<FILE*>(user_data); }
	 ...
	 *
	 * This function writes a complete log entry to a file sink by concatenating the message's
	 * preamble, indentation, prefix, and main text, followed by a newline. When compiled with
	 * file abstraction enabled (LOGURU_WITH_FILEABS), it first checks if the file requires reopening
	 * (to account for external modifications) and performs the reopen if necessary, unless the file
	 * is already in the process of reopening. If a valid file pointer is not available, logging is skipped.
	 * The file is flushed immediately if the global flush interval is set to zero.
	 *
	 * @param user_data Pointer to a file abstraction or handle used to manage file logging state.
	 * @param message Structure containing the components of the log message.
	 */
	...
	/**
	 * @brief Closes the file associated with the provided user data.
	 *
	 * This function converts the given user data into a file pointer and, if valid, closes the file. When
	 * compiled with LOGURU_WITH_FILEABS enabled, it additionally frees the memory allocated for the associated
	 * FileAbs structure.
	 *
	 * @param user_data Pointer to file-related data, which should be a valid FILE pointer or a FileAbs instance.
	 */
	...
	/**
	 * @brief Flushes the output buffer of the file associated with the given user data.
	 *
	 * This function converts the provided user data to a FILE pointer and flushes its output buffer.
	 *
	 * @param user_data Pointer to the file object used to manage file logging.
	 */
...
#if LOGURU_WITH_FILEABS
	/**
	 * @brief Reopens a log file if it is closed, has encountered an error, or has changed.
	 *
	 * This function inspects the log file state encapsulated in the FileAbs structure (provided via
	 * the user_data pointer). It compares the current file status with previously recorded metadata. If 
	 * the file pointer is invalid, an error occurs when retrieving the file status, or the file has changed 
	 * (as indicated by a differing inode), the function closes the current file (if applicable), attempts 
	 * to recreate necessary directories, and reopens the file. Upon successful reopening, the file metadata 
	 * is refreshed, and the reopening flag is cleared. All significant events are logged appropriately.
	 *
	 * @param user_data Pointer to a FileAbs structure holding details about the log file, including its 
	 *                  path, mode string, file pointer, and associated metadata.
	 */
	void file_reopen(void* user_data)
	{
		FileAbs * file_abs = reinterpret_cast<FileAbs*>(user_data);
...
#if LOGURU_SYSLOG
	/**
	 * @brief Logs a message to the syslog daemon with an appropriate priority.
	 *
	 * This function converts the internal log message's verbosity level into the corresponding syslog
	 * priority level and sends the formatted log entry to the syslog daemon, which automatically adds
	 * timestamp information. The log entry is composed by concatenating the message's indentation, prefix,
	 * and main text. Verbosity levels lower than Verbosity_FATAL are treated as System Alerts (priority 1),
	 * while higher verbosity levels are mapped as follows:
	 * - Verbosity_FATAL   → Critical (priority 2)
	 * - Verbosity_ERROR   → Error (priority 3)
	 * - Verbosity_WARNING → Warning (priority 4)
	 * - Verbosity_INFO    → Notice (priority 5)
	 * - Verbosity_1       → Info (priority 6)
	 * - Any other value   → Debug (priority 7)
	 *
	 * @param message The log message containing metadata (verbosity, indentation, prefix) and the message text.
	 */
	...
	/**
	 * @brief Closes the syslog logging connection.
	 *
	 * This function serves as a callback that terminates the system log session by invoking the standard 
	 * closelog() function. The provided user data parameter is not used.
	 */
	...
	/**
	 * @brief No-op flush for syslog logging sinks.
	 *
	 * This function is a placeholder that fulfills the flush callback interface.
	 * Since syslog handles output immediately, explicit flushing is not needed.
	 *
...
// ------------------------------------------------------------------------------
	/**
 * @brief Destroys the Text instance.
 *
 * Frees the memory allocated for the internal string, ensuring proper cleanup.
 */
	...
	/**
	 * @brief Returns a Text object with formatted content.
	 *
	 * Uses the provided format string and arguments to generate a formatted string,
	 * which is then encapsulated in a Text object.
	 *
	 * @param format The C-string format specifier.
	 * @param args The arguments corresponding to the format specifier.
	 * @return A Text object containing the formatted string.
	 */
	...
	/**
	 * @brief Creates a formatted Text object from a format string and a variable argument list.
	 *
	 * This function constructs a Text object by formatting the provided C-style string using values
	 * from the given va_list. It handles memory allocation and formatting using platform-specific routines:
	 * - On Windows, it determines the required buffer size with _vscprintf and formats the string with vsnprintf.
	 * - On other platforms, it leverages vasprintf to allocate and format the buffer.
	 *
	 * @param format A C-style format string.
	 * @param vlist A va_list containing the arguments to be formatted.
	 * @return Text A Text object containing the formatted string.
	 */
	...
	/**
	 * @brief Formats a string using printf-style formatting and returns a Text object.
	 *
	 * This function accepts a format string followed by a variable number of arguments,
	 * formats the text accordingly, and returns the resulting Text object.
	 *
	 * @param format The C-string format that specifies how to format the arguments.
	 * @return A Text object containing the formatted string.
	 */
	...
	/**
	 * @brief Overload of textprintf for variadic template matching.
	 *
	 * This overload returns an empty Text object by allocating a single zero-initialized character.
	 * It serves as the base case in variadic template formatting, where no formatting arguments are provided.
	 *
	...
	/**
	 * @brief Returns a pointer to a precomputed indentation string.
	 *
	 * This function uses a static buffer containing repeated indentation patterns (".   ") to
	 * provide an indentation string for the specified depth. It returns a pointer to the substring
	 * representing the requested number of indentation levels. If the provided depth exceeds the
	 * available indentations, the maximum indentation available is returned.
	 *
	 * @param depth The number of indentation levels desired.
	 * @return const char* Pointer to the indentation substring corresponding to the specified depth.
	 */
	...
	/**
	 * @brief Parses command-line arguments to extract and process the verbosity flag.
	 *
	 * This function iterates through the provided command-line arguments (argv) and removes any that match the specified verbosity flag. When a matching flag is found, it extracts the associated verbosity level—either from the remainder of the same argument or from the following argument—and sets the logging verbosity accordingly via a global variable. The argument count (argc) is updated to reflect the removal of processed verbosity arguments.
	 *
	 * @param argc Reference to the argument count; modified to exclude processed verbosity flags.
	 * @param argv Array of command-line arguments; entries corresponding to the verbosity flag are removed.
	 * @param verbosity_flag C-string that designates the prefix used to specify the verbosity level.
	 */
	 ...
	 *
	 * This function obtains the current time using the high-resolution clock and converts it to the number of nanoseconds elapsed since the epoch.
	 *
	 ...
	 *
	 * This function scans through the given path and returns a pointer to the substring
	 * immediately following the last occurrence of '/' or '\\'. If neither delimiter is found,
	 * the entire path is returned, implying it contains no directory components.
	 *
	 * @param path A C-string representing the file path.
	 * @return const char* A pointer to the filename within the input path.
	 */
	...
	/**
	 * @brief Atexit handler that logs a termination message and flushes the logging buffers.
	 *
	 * This function is registered to be called when the program exits. It logs an "atexit" message
	 * at the internal verbosity level and then calls flush() to ensure that all pending log messages
	 * are written out before the application terminates.
	 */
	...
	/**
	 * @brief Appends the hexadecimal digit representation of a number to a string.
	 *
	 * Converts the given number (which must be less than 16) to its corresponding hexadecimal digit
	 * and appends it to the end of the provided string. For numbers from 0 to 9, the function appends 
	 * the numeral ('0'-'9'); for numbers from 10 to 15, it appends the corresponding uppercase letter ('A'-'F').
	 *
	 * @param out The string to which the hexadecimal digit is appended.
	 * @param num The numeric value to convert; must be less than 16.
	 */
	...
	/**
	 * @brief Appends the two-digit hexadecimal representation of a byte to a string.
	 *
	 * The function converts an 8-bit unsigned integer into its hexadecimal representation
	 * by extracting the high and low 4-bit nibbles and appending each corresponding hexadecimal
	 * digit to the output string.
	 *
	 * @param out The string that will receive the hexadecimal digits.
	 * @param n The byte value to convert into hexadecimal form.
	 */
	...
	/**
	 * @brief Escapes special and control characters in the input string.
	 *
	 * Iterates over each character in the given string and appends its escaped representation to the output
	 * string. Recognized characters (e.g., newline, tab, backslash, quotes, space) are replaced with their 
	 * corresponding escape sequences. Control characters with ASCII values in the range 0x00 to 0x1F that are not
	 * otherwise handled are converted into a hexadecimal escape sequence.
	 *
	 * @param out Output string that accumulates the escaped characters.
	...
	/**
	 * @brief Converts the global errno value into a human-readable error message.
	 *
	 * This function returns a newly allocated Text object containing the error message
	 * corresponding to the current errno value. It uses platform-specific implementations
	 * to provide an appropriate error description, ensuring thread safety on systems that
	 * support the GNU or XSI versions of strerror_r.
	 *
	 * @return Text A Text object holding the error message.
	 */
	Text errno_as_text()
	{
		char buff[256];
	 ...
	 *
	 * This function sets up the logging infrastructure by verifying the command-line arguments,
	 * capturing the current working directory, and concatenating the arguments into a single string.
	 * If a verbosity flag is provided, it parses and applies verbosity settings. It also assigns the 
	 * main thread's name (if specified) using platform-specific APIs and prints a preamble header to stderr 
	 * when the log verbosity is sufficiently high. Additionally, it installs signal handlers based on the 
	 * provided options and registers a cleanup function to be invoked at program exit.
	 *
	 * @param argc Reference to the argument count; must be greater than zero with argv[argc] equal to nullptr.
	 * @param argv Array of C-style strings representing the command-line arguments.
	 * @param options Logging options including verbosity flag, main thread name, and signal handler configurations.
	 */
	 ...
	 *
	 * Cleans up internal logging resources by removing all registered callbacks and resetting
	 * the fatal and verbosity-related handler callbacks to their default (null) states.
	 * This function should be called when the logging system is no longer needed.
	 */
	void shutdown()
	{
	...
	/**
	 * @brief Writes the current local date and time into a buffer.
	 *
	 * This function retrieves the current system time, converts it to local time, and formats it
	 * as a string in the "YYYYMMDD_HHMMSS.mmm" format (where "mmm" represents milliseconds). The
	 * resulting string is then written into the provided buffer.
	 *
	 * @param buff Pointer to the character buffer where the formatted date and time string will be stored.
	...
	/**
	 * @brief Retrieves the program's executable filename.
	 *
	 * Returns a C-style string containing the basename of the executable as provided by argv[0].
	 * This is typically used for logging and error reporting.
	 *
	 * @return const char* Pointer to the executable's filename.
	 */
	...
	/**
	 * @brief Retrieves the stored command line arguments as a C-string.
	 *
	 * This function returns a pointer to the null-terminated string holding the command line arguments.
	 * The returned pointer remains valid as long as the internal argument string is unchanged.
	 *
	 * @return const char* Pointer to the command line arguments.
	 */
	...
	/**
	 * @brief Retrieves the current working directory used by the logging system.
	 *
	 * This function returns a pointer to the internal string that holds the current working
	 * directory, which is utilized for resolving relative paths in file-based logging operations.
	 * The returned string should not be modified.
	 *
	 * @return const char* A pointer to the current working directory.
	 */
	 ...
	 *
	 * This function obtains the home directory path from the environment. On Windows systems, it looks for
	 * the "USERPROFILE" variable, while on Unix-like systems it retrieves the "HOME" variable. The function
	 * aborts execution if the expected environment variable is not set.
	 *
	 * @return const char* A pointer to the user's home directory.
	 */
	const char* home_dir()
	{
		#ifdef __MINGW32__
	...
	/**
	 * @brief Generates a suggested log file path for log output.
	 *
	 * This function constructs a complete log file path using the provided directory prefix. If the prefix starts
	 * with '~', it is expanded to the user's home directory. The function ensures the path ends with a trailing slash,
	 * then appends the executable's name, a directory separator, the current date and time, and finally the ".log" extension.
	 *
	 * @param prefix The base path for the log file. A '~' as the first character indicates the home directory should be used.
	 * @param buff Buffer to store the generated log file path.
	...
	/**
 * @brief Registers a file as a logging sink.
 *
 * This function resolves the provided file path—replacing a leading '~' with the user's home directory—and ensures that
 * necessary directories exist. It opens the file in either truncate (write) or append mode, writes initial logging context
 * (such as command-line arguments, current directory, and verbosity level), and registers the file as a callback for log messages
 * at the specified verbosity level.
 *
 * @param path_in File path string; a leading '~' is replaced with the user's home directory.
 * @param mode FileMode indicating whether to truncate or append to the file.
 * @param verbosity Minimum verbosity level for messages written to the file.
 * @return true if the file is successfully opened and registered; false if the file cannot be opened.
 *
 * @note If directory creation fails, an error is logged. In append mode, additional newlines are written to separate log entries.
 */
...
namespace loguru {
	/**
	 * @brief Installs signal handlers based on the provided options.
	 *
	 * Configures process signal handlers according to the specified SignalOptions. Note that
	 * on Windows, signal handling is not yet implemented.
	 *
	 * @param signal_options The configuration options specifying which signals to handle.
	 */
...
{
	/**
	 * @brief Writes a block of data to the standard error output.
	 *
	 * Writes up to @c size bytes from the given data buffer to the standard error file descriptor.
	 * Any errors encountered during the write operation are intentionally ignored.
	 *
	...
	/**
	 * @brief Writes a null-terminated string to standard error.
	 *
	 * This function outputs the provided null-terminated string to stderr by calculating its length using
	 * strlen and passing both the string and its length to the underlying implementation.
	 *
	 * @param data Pointer to the null-terminated string to be written.
	 */
	...
	/**
	 * @brief Restores the default handler for a given signal and triggers it.
	 *
	 * This function resets the specified signal's handler to its default behavior by configuring
	 * the signal action, and then sends the signal to the current process. Use this to enforce
	 * the system's standard response to a signal.
	 *
	 * @param signal_number The signal number to be reset and reissued.
	 */
	...
	/**
	 * @brief Handles an incoming signal by logging its occurrence and delegating to the default handler.
	 *
	 * This function maps the received signal number to a human-readable name and writes a formatted message to stderr,
	 * using terminal color formatting if enabled. When unsafe signal handling is activated via configuration, it flushes log
	 * buffers and logs a fatal message—including a preamble and stack trace—before deferring to the system's default signal handler.
	 *
	 * @param signal_number The numeric identifier of the caught signal.
	 * @param siginfo Pointer to a siginfo_t structure with additional signal information (unused).
	 * @param ucontext Pointer to a context-specific structure (unused).
	 */
	...
	/**
	 * @brief Installs custom signal handlers based on the provided options.
	 *
	 * This function configures and registers a common signal handler for each signal specified
	 * in the provided SignalOptions. It sets up a sigaction structure with the SA_SIGINFO flag
	 * and assigns the designated signal handler function. For each enabled signal (e.g., SIGABRT,
	 * SIGBUS, SIGFPE, SIGILL, SIGINT, SIGSEGV, SIGTERM), the function attempts to install the handler,
	 * with runtime checks ensuring successful registration.
	 *
	 * @param signal_options Options that specify which signals should have handlers installed.
	 */
	void install_signal_handlers(const SignalOptions& signal_options)
	{
